/**
 * ملف: mlbb-quiz.validation.ts
 * الغرض: دوال تحقّق (Validation) خاصة بوحدة لعبة "كويز موبايل ليجندز" فقط.
 *
 * نفس فكرة flags.validation.ts وriddles.validation.ts بالضبط: نتأكد أن
 * كائن قادم من ملف JSON فعلًا يطابق شكل MlbbQuizQuestion الصحيح قبل أن
 ...
 * عنصر في questions.json نفسه (نفس أسلوب flags.validation.ts)، وليس
 * مولَّدًا تلقائيًا وقت التحميل. التحقق من الفرادة (uniqueness) بين كل
 * العناصر يحدث لاحقًا في mlbb-quiz.service.ts (loadMlbbQuizQuestions)،
...
export interface MlbbQuizValidationResult {
  valid: boolean;
  errors: string[];
}

/**
 * تتحقق أن قيمة واحدة قادمة من JSON هي فعلًا سؤال "كويز موبايل
 * ليجندز" صالح الشكل، وتُرجع قائمة أخطاء واضحة (بدل true/false فقط)
 ...
 *   4. نتأكد أن answer نص غير فارغ (بعد trim).
 */
export function validateMlbbQuizQuestion(value: unknown): MlbbQuizValidationResult {
  const errors: string[] = [];
  ...
  if (typeof item.answer !== "string" || item.answer.trim() === "") {
    errors.push("حقل answer مفقود أو فارغ أو ليس نصًا");
  }
 ...
 */
export function isValidMlbbQuizQuestion(value: unknown): value is MlbbQuizQuestion {
  return validateMlbbQuizQuestion(value).valid;
}
