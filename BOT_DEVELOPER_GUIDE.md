...
استبدلي `<your-deployment>` برابط النشر الفعلي بتاعك على Vercel. كل الأمثلة تحت مبنية على إن عندك متغيّر زي:

```js
const API_BASE = process.env.RAYGUMO_API_URL ?? "https://<your-deployment>.vercel.app";
```
...
### مثال: عدد الأسئلة
```bash
curl https://<your-deployment>.vercel.app/api/games/quiz/count
```
```json
