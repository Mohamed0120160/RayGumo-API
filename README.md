...
هذا المشروع في مرحلة **MVP (نسخة أولية قابلة للاستخدام)**. حاليًا يقدّم خمس ألعاب — **Quiz (كويز)**، **True or False (صح أو خطأ)**، **Riddles (ألغاز)**، **Eye (عين)**، و**Emoji Game (خمّن الشخصية من الإيموجي)** — والبنية المعمارية مصمَّمة عمدًا بحيث إضافة ألعاب جديدة مستقبلًا لا تتطلب أي تعديل في بنية routes الـ API، بل فقط وحدات ألعاب جديدة مستقلة بذاتها.
...
> للتوثيق الكامل والمفصّل لهذه اللعبة (كل الـ endpoints، أمثلة كاملة، تكامل Baileys جاهز، وشرح `random-exclude`) راجعي [`docs/emoji.md`](./docs/emoji.md).

### وصف اللعبة

لعبة "إيموجي" (Emoji Game) تعرض مجموعة إيموجيات تمثّل شخصية أنمي أو شخصية عامة (مثل `🍖 👒 🏴‍☠️`)، والمطلوب من اللاعب تخمين اسم هذه الشخصية. بالضبط مثل الكويز والألغاز، سؤال واحد قد يقبل أكثر من صياغة صحيحة للإجابة (عربي/إنجليزي أو أكثر من اسم)، لذلك الإجابات تُمثَّل كمصفوفة نصوص (`answers: string[]`).

مصدر بيانات "إيموجي" موجود في ملف واحد:

```
src/data/emoji/questions.json
```

عبارة عن مصفوفة JSON فيها 500 سؤال حقيقي. كل سؤال مطابق للـ schema ده:
...
- `id` — رقم صحيح موجب فريد. **مخزَّن مسبقًا ومرقّم تسلسليًا (1 إلى 500) داخل questions.json نفسه** (نفس أسلوب `riddles` و`eye`)، وليس مولَّدًا وقت التحميل. عند إضافة أسئلة جديدة يدويًا للملف، لازم يُعطى كل سؤال جديد `id` فريدًا لم يُستخدم من قبل (عادة: أكبر `id` موجود حاليًا + 1).
- `emoji` — نص يحتوي الإيموجيات الممثِّلة للشخصية. دعم كامل لـ UTF-8 وتسلسلات الإيموجي المركّبة.
- `category` — تصنيف السؤال (حاليًا اسم الأنمي في كل البيانات المزوَّدة). هذا الحقل حلّ محل حقل `anime` الذي كان موجودًا في ملف البيانات الخام الأصلي.
- `answers` — **مصفوفة نصوص**، تدعم العربية والإنجليزية وأي مرادفات مطلوبة لنفس الشخصية.

أي سجل ناقص فيه `id` أو `emoji` أو `category` أو `answers` (كمصفوفة غير فارغة من نصوص غير فارغة) بيتجاهل تلقائيًا وقت التحميل ومش هيظهر في أي endpoint. لو اتكرر نفس الـ `id` بين سؤالين، أول سؤال بيفوز والباقي بيتجاهل مع تحذير في الـ logs (نفس سلوك `riddles` و`eye` بالضبط).

### الـ Endpoints

لعبة "إيموجي" متاحة على نفس المسار العام المشترك بين كل الألعاب: `/api/games/emoji/...` (نفس نمط `quiz` و`riddles` و`eye` تمامًا).

| المسار (Endpoint) | Method | الوصف |
|---|---|---|
| `/api/games/emoji/random` | GET | ترجّع سؤالًا عشوائيًا واحدًا |
| `/api/games/emoji/random-exclude?ids=1,5,8,20` | GET | ترجّع سؤالًا عشوائيًا واحدًا مع استثناء الـ ids الممرّرة (anti-repeat) |
| `/api/games/emoji/all` | GET | ترجّع كل الأسئلة |
| `/api/games/emoji/count` | GET | ترجّع العدد الإجمالي للأسئلة |
| `/api/games/emoji/:id` | GET | ترجّع سؤالًا واحدًا بواسطة رقمه (id) |

#### `GET /api/games/emoji/random`

```bash
curl http://localhost:3000/api/games/emoji/random
```
...
ترجّع سؤالًا عشوائيًا واحدًا، مع استثناء أي `id` ممرّرة في معامل الاستعلام `ids` (مفصولة بفواصل). نفس مبدأ `riddles` و`eye` بالضبط.

```bash
curl "http://localhost:3000/api/games/emoji/random-exclude?ids=1,5,8,20"
```

لو `ids` غير موجود أو فارغ، السلوك بيبقى مطابق تمامًا لـ `/random`. لو كل الأسئلة المتاحة استُثنيت، الـ endpoint بيرجّع خطأ `404 NOT_FOUND`:
...
```bash
curl http://localhost:3000/api/games/emoji/all
```

ترجّع مصفوفة `questions.json` كاملة (500 سؤال) داخل الشكل الموحّد.

#### `GET /api/games/emoji/count`

```bash
curl http://localhost:3000/api/games/emoji/count
```
...
```bash
curl http://localhost:3000/api/games/emoji/15
```

أي `id` مفقود/غير صالح بيرجّع خطأ `400 BAD_REQUEST`، وأي `id` غير موجود بيرجّع `404 NOT_FOUND`.

### نظام منع التكرار (anti-repeat)

نفس نمط باقي الألعاب بالضبط — استخدمي `GET /api/games/emoji/random-exclude?ids=1,2,3` بدل `/random` العادي، وتتبّعي أرقام الأسئلة (`id`) المستخدمة لكل مجموعة في البوت نفسه. أكواد جاهزة كاملة لهذا النمط (وباقي أمثلة التكامل مع Baileys) موجودة في [`docs/emoji.md`](./docs/emoji.md).

لتحديث المحتوى: عدّلي `src/data/emoji/questions.json`، اعملي commit، وارفعي (redeploy) — بدون أي تعديل كود.

---

## Sort API (رتب)

> للتوثيق الكامل والمفصّل لهذه اللعبة (كل الـ endpoints، شرح `random-exclude`، ولماذا `answers` غير موجودة في `random`) راجعي [`docs/sort.md`](./docs/sort.md).

### وصف اللعبة

لعبة "رتب" تعرض على اللاعب سطر حروف مبعثرة (مثل `"ن ا ج ي ن د ا"`)، وعليه إعادة ترتيبها للوصول إلى الكلمة الصحيحة (`"ناجيندا"`). كل سؤال يحمل أكثر من ترتيب عشوائي مختلف لنفس الحروف، والـ API يختار سطرًا واحدًا فقط منها عند كل طلب عشوائي.

مصدر بيانات "رتب" موجود في ملف واحد:
...
{
  "id": 419,
  "questions": ["ن ا ج ي ن د ا", "د ن ا ج ي ن ا", "ي ا ن د ا ج ن", "ج ا د ن ا ي ن"],
  "answers": ["ناجيندا"]
}
```

- `id` — رقم صحيح موجب فريد. **مخزَّن مسبقًا داخل questions.json نفسه** (نفس أسلوب `riddles` و`eye` و`emoji`)، وليس مولَّدًا وقت التحميل.
- `questions` — مصفوفة نصوص، كل نص ترتيب حروف مبعثر مختلف لنفس الكلمة. يُختار عنصر واحد منها عشوائيًا عند كل طلب `random`/`random-exclude`.
- `answers` — مصفوفة نصوص، الإجابة/الإجابات الصحيحة المقبولة.

**الفرق الجوهري عن باقي الألعاب:** استجابة `random` و`random-exclude` هنا لا تحتوي أبدًا على `answers` — فقط `{ id, question }` — حتى لا تُكشَف الإجابة للبوت أثناء اللعب. أما `all` و`:id` فيرجعان العنصر كاملًا بما فيه `answers`. راجعي [`docs/sort.md`](./docs/sort.md#لماذا-answers-غير-موجودة-في-random-وrandom-exclude) للتفاصيل.

### الـ Endpoints

لعبة "رتب" متاحة على نفس المسار العام المشترك بين كل الألعاب: `/api/games/sort/...`.

| المسار (Endpoint) | Method | الوصف |
|---|---|---|
| `/api/games/sort/random` | GET | ترجّع سؤالًا عشوائيًا واحدًا (`{ id, question }`، بدون `answers`) |
| `/api/games/sort/random-exclude?ids=1,5,8,20` | GET | نفس `random`، مع استثناء الـ ids الممرّرة (anti-repeat) |
| `/api/games/sort/all` | GET | ترجّع كل الأسئلة كاملة (بما فيها `questions` و`answers`) |
| `/api/games/sort/count` | GET | ترجّع العدد الإجمالي للأسئلة |
| `/api/games/sort/:id` | GET | ترجّع سؤالًا واحدًا كاملًا بواسطة رقمه (id) |

#### `GET /api/games/sort/random`

```bash
curl http://localhost:3000/api/games/sort/random
```

```json
{
  "success": true,
  "data": { "id": 419, "question": "ن ا ج ي ن د ا" }
}
```
...
```bash
curl "http://localhost:3000/api/games/sort/random-exclude?ids=1,5,8,20"
```
...
```bash
curl http://localhost:3000/api/games/sort/all
```

ترجّع مصفوفة `questions.json` كاملة (500 سؤال) داخل الشكل الموحّد، كل سؤال بما فيه `questions` و`answers`.

#### `GET /api/games/sort/count`

```bash
curl http://localhost:3000/api/games/sort/count
```
...
```bash
curl http://localhost:3000/api/games/sort/419
```
...
بنية الـ routes (`/api/games/[game]/...`) وطبقة التخزين (`lib/json-db.ts`) عامة (generic) بالفعل. ألعاب `true-false` (صح أو خطأ) و`riddles` (ألغاز) و`eye` (عين) و`emoji` (إيموجي) مبنية بالضبط بهذا الأسلوب، ويمكن اعتبار أيّ منها مثالًا حيًا كاملًا لإضافة لعبة جديدة. لإضافة لعبة جديدة أخرى (مثلًا `anime`):
